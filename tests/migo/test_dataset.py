...
def make_dataset(
    *, batch_with_collate: bool = False, ignore_opening_moves: int = 0
) -> migo.SgfDataset:
...
def test_sign():
    ...
    N = len(dataset)
    # first game +B
    _x, _move, value = dataset[0]
    assert value == 1
    _x, _move, value = dataset[1]
    assert value == -1
    _x, _move, value = dataset[2]
    assert value == 1
    # second game draw
    _x, _move, value = dataset[N - 1]
    assert value == 0
    _x, _move, value = dataset[N - 2]
    assert value == 0
...
def test_append():
    ...
    for i in range(1, len(games)):
        other.append(migo.SgfDataset(games=[games[i]], history_n=1))
...
def test_append_with_opening():
    ign = 1
    games = [migo.record.parse_sgf_game(_) for _ in short_sgf_str]
    all = migo.SgfDataset(games=games, history_n=1, ignore_opening_moves=ign)
    assert len(all) == 4

    other = migo.SgfDataset(
        games=[games[0]], history_n=1, ignore_opening_moves=ign
    )
    for i in range(1, len(games)):
        other.append(
            migo.SgfDataset(
                games=[games[i]], history_n=1, ignore_opening_moves=ign
            )
        )
    assert all.n_games() == other.n_games()
    assert len(all) == len(other)
...
def test_collate_extended():
    ...
    aux_zones[0, 1, 2] = 1
    aux_zones[1, 5, 4] = 1
    aux_scores = np.arange(2 * base.n_games(), dtype=np.float32).reshape(
        2, -1
    ) / (2 * base.n_games())
...
def test_append_extended():
    ...
    aux_zones[0, 1, 2] = 1
    aux_zones[1, 5, 4] = 1
    aux_scores = np.arange(2 * all_base.n_games(), dtype=np.float32).reshape(
        2, -1
    ) / (2 * all_base.n_games())
    ...
    for i in range(1, len(games)):
        base = migo.SgfDataset(games=[games[i]], history_n=1)
...
def test_flip_lr():
    ...
    assert np.array_equal(
        bd, np.array([[[2, 1, 0], [5, 4, 3], [8, 7, 6]]]) + offset
    )
...
def test_flip_lr_torch():
    from migo.dataset_transform import flip_lr

    offset = 100
    bd = torch.from_numpy(np.arange(offset, offset + 9).reshape(1, 3, 3))
    id = torch.from_numpy(np.array([list(range(9)) + [-1]]))
    flip_lr(3, bd, id)
    assert np.array_equal(
        bd, np.array([[[2, 1, 0], [5, 4, 3], [8, 7, 6]]]) + offset
    )
    ...
    bd = torch.from_numpy(np.arange(9).reshape(1, 3, 3))
    aux_bd = torch.from_numpy(np.arange(offset, offset + 10).reshape(1, 10))
    id = torch.from_numpy(np.array([list(range(9)) + [-1]]))
    flip_lr(3, bd, id, aux_bd)
    assert np.array_equal(bd, [[[2, 1, 0], [5, 4, 3], [8, 7, 6]]])
    assert np.array_equal(id, id2)
...
def test_flip_udlr():
    ...
    assert np.array_equal(bd, np.array(rev).reshape(1, 3, 3) + offset)
    id2 = np.array([rev + [9]])
    assert np.array_equal(id, id2)
    ...
    assert np.array_equal(aux_bd, aux_bd2)


def test_flip_udlr_torch():
    from migo.dataset_transform import flip_udlr
...
def test_flip_ud():
    ...
    assert np.array_equal(bd, np.array(bd_v).reshape(1, 3, 3) + offset)
    id2 = np.array([bd_v + [9]])
    assert np.array_equal(id, id2)
...
def test_rot90():
    ...
    assert np.array_equal(bd, np.array(bd90).reshape(1, 3, 3) + offset)
...
def test_flip_move_19x19():
    from migo.dataset_transform import flip_ident

    moves = np.arange(-1, 19**2, dtype=np.int16)
    board = np.zeros(19**2).reshape(1, 19, 19)
    before = np.copy(moves)
    before[0] = 19**2  # pass
    flip_ident(19, board, moves)
    print(before)
    print(moves)
    assert np.array_equal(before, moves)  # should be identical

    from migo.dataset_transform import flip_lr, flip_ud, flip_udlr, rot90

    transforms = [flip_lr, flip_udlr, flip_ud, rot90]
    for tf in transforms:
        ...
        before.sort()
        moves.sort()
        assert np.array_equal(before, moves)  # one-to-one mapping


def test_dataset_mixer():
    srcdb = make_dataset()
    singleton_mixer = migo.dataset.DatasetMixer([srcdb])
    assert len(srcdb) == len(singleton_mixer)
    batch_a = srcdb.collate([0])
    batch_b = singleton_mixer.collate([0])
    assert len(batch_a) == len(batch_b)
    assert batch_a[0].shape == batch_b[0].shape
    assert batch_a[1].shape == batch_b[1].shape
    assert batch_a[2].shape == batch_b[2].shape
    assert torch.equal(batch_a[0], batch_b[0])
    assert torch.equal(batch_a[1], batch_b[1])
    assert torch.equal(batch_a[2], batch_b[2])
