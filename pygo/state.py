...
from pygo.misc import all_coordinates, Color, Coord, Move, IllegalMoveError, \
    Pass, index_to_coord
...
class State:
    ...
    def to_cygo(self):
        ...
        >>> state, _ = pygo.state.parse('. B . B|. B . B|. W W .|. . . .|')
        >>> print(state)
            A  B  C  D 
           ------------
         4| .  .  .  . |
        ...
        1 .  X  .  X 1
          A  B  C  D 
        ...
        >>> for v in pygo.all_coordinates(state.board_size):
...
def parse(board_str: str, next_color: Color = None,
    ...
    ...     '. B W . |',
    ... ])
    >>> state, moves = pygo.state.parse(board)
    >>> print(state)
        A  B  C  D 
       ------------
     4| .  B  W  . |
    ...
    >>> moves['a']
    (0, 0)
    >>> state.is_suicide_move(moves['a'], pygo.Color.BLACK)
    False
    >>> state.is_suicide_move(moves['a'], pygo.Color.WHITE)
    True
    """
