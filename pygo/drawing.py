...
"""
from pygo import Color, State, all_coordinates
...
def make_fig(board_size: int = 9,
    ...
    >>> fig, ax, ax_note = pygo.drawing.make_fig(9)
    >>> assert fig is not None
    >>> assert ax is not None
    >>> assert ax_note is None
    >>> fig, ax, ax_note = pygo.drawing.make_fig(9, with_note=True)
    >>> assert ax is not None
...
def place_stones(ax, state: State | cygo.State,
    ...
    :param state: :py:class:`pygo.State` or :py:class:`cygo.State`

    >>> state = pygo.State(9)
    >>> state.make_move((4, 4))
    False
    >>> fig, ax, _ = pygo.drawing.make_fig(state.board_size)
    >>> pygo.drawing.place_stones(ax, state)
    <Axes:...
    """
    planes = pygo.features.board(state)
...
def plot_record(record, config: dict = {}, *,
    ...
    if overwrite:
        ...
        state_planes = pygo.features.board(state)
        black_index = (1 - int(state.current_player)) // 2
