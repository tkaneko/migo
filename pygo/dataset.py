...
class SgfDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, flat_idx):
        ...
        xh = pygo.features.history_n(state, self.history_n)
    ...
    def to_img(self, flat_idx):
        ...
        state = cygo.State(self.board_size, max_history_n=self.history_n)
        cygo.apply_moves(state, moves[:move_id])
        x = pygo.features.history_n(state, self.history_n)
        ...
        # draw main board
        pygo.drawing.setup_board_ax(self.board_size, fig, axs[0])
        scale = pygo.drawing.ax_scale(9, fig, axs[0])
        pygo.drawing.place_stones(axs[0], state, scale=scale)
        pygo.drawing.put_number(
        ...
        for i, plane in enumerate(x):
            pygo.drawing.setup_board_ax(self.board_size, fig, axs[i+1])
            color = cygo.Color.BLACK if i % 2 == 0 else cygo.Color.WHITE
            pygo.drawing.draw_plane(axs[i+1], plane, color,
                                    scale=scale)
            pygo.drawing.put_number(
    ...
    def load_from(path):
        ...
        usage: `dataset = pygo.dataset.SgfDataset('path-to-npz')`
        """
        objs = np.load(path)
        ret = SgfDataset('')
