...
from . import cygo, features
from .record import SimpleRecord, load_sgf_games_in_folder
...
class SgfDataset(torch.utils.data.Dataset):
    ...
    def nth_game(self, n: int) -> SimpleRecord:
    ...
    def moves_view(self, game_id) -> np.ndarray:
        ...
        lt, rt = self.move_offset[game_id], self.move_offset[game_id + 1]
        return self.game_moves[lt:rt]
    ...
    def __getitem__(self, index: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor] | int:
        ...
        xh = features.history_n(state, self.history_n)
        xc = features.color(state)
        x = np.vstack((xh, xc))
    ...
    def set_transform(self, seed: int | str | None):
    ...
    def to_img(self, flat_idx):
        ...
        state = cygo.State(self.board_size, max_history_n=self.history_n)
        cygo.apply_moves(state, moves[:move_id])
        x = features.history_n(state, self.history_n)
        fig, axs = plt.subplots(
            1, 1 + len(x), figsize=((1 + len(x)) * 3.3, 3.7)
        )
        cmove = cygo.Move.from_raw_value(
            moves[move_id], board_size=self.board_size
        )
        coord = (cmove.row, cmove.col)
        # draw main board
        drawing.setup_board_ax(self.board_size, fig, axs[0])
        scale = drawing.ax_scale(9, fig, axs[0])
        drawing.place_stones(axs[0], state, scale=scale)
        drawing.put_number(
            axs[0],
            self.board_size,
            'k',
        ...
        for i, plane in enumerate(x):
            drawing.setup_board_ax(self.board_size, fig, axs[i + 1])
            color = cygo.Color.BLACK if i % 2 == 0 else cygo.Color.WHITE
            drawing.draw_plane(axs[i + 1], plane, color, scale=scale)
            drawing.put_number(
                axs[i + 1],
                self.board_size,
                'k',
    ...
    def npy_to_img(self, flat_idx):
        ...
        grid = ImageGrid(
            fig, 111, nrows_ncols=(nrows, ncols), axes_pad=0.3, label_mode='all'
        )
    ...
    def build_from_folder(folder_path: str | list[str], *, batch_with_collate):
        ...
        for folder in folder_path:
            ...
            if not board_size:
                ...
                raise RuntimeError(
                    f'board size mismatch {board_size}'
    ...
    def summary(self) -> dict:
        ...
        for i in range(n_games):
            data = self.moves_view(i)
            key = hashlib.sha256(data).digest()
            path_count[key] = path_count.get(key, 0) + 1
        n_unique = len(path_count)
        return {
            'dbtype': 'SgfDataset',
            'games': n_games,
            '#uniq': n_unique,
            ...
            'black-win': np.count_nonzero(self.winner == 1)
            + 0.5 * np.count_nonzero(self.winner == 0),
        }
    ...
    def append(self, other) -> None:
        ...
        self.move_offset = np.append(
            self.move_offset[:-1], other.move_offset + offset
        )
        offset_data = self.data_offset[-1]
        self.data_offset = np.append(
            self.data_offset[:-1], other.data_offset + offset_data
        )
...
class ExtendedDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        self,
        sgfdataset: SgfDataset,
        aux_input_planes,
        aux_values,
        aux_oplanes: np.ndarray | None = None,
    ):
        """
        ...
        if (
            abs_max := max(abs(aux_values[0]).max(), abs(aux_values[1]).max())
        ) > 1:
        ...
        self.enabled_colors: list[int] = [True, True]
    ...
    def nth_game(self, n: int) -> SimpleRecord:
        return self.sgf_dataset.nth_game(n)
    ...
    def _collate_naive(self, indices):
        ...
        x = np.zeros(
            (N, channels_per_state, board_size, board_size), dtype=float
        )
        ...
        for i, flat_idx in enumerate(indices):
            ...
            cygo.apply_moves(state, moves[:move_id])
            xh = features.history_n(state, history_n)
            xc = features.color(state)
    ...
    @staticmethod
    def build_from(base: SgfDataset, zones):
        board_size = base.board_size
        ...
        zone_score[1] = (
            -np.dot(  # as white stone be -1 in territory
...
def load_dataset(
    path_or_objs, *, batch_with_collate
) -> ExtendedDataset | SgfDataset:
    """load either ExtendedDataset or SgfDataset"""
    objs = path_or_objs
...
class DatasetMixer(torch.utils.data.Dataset):
    def __init__(self, dbs):
        self.dbs = dbs
        self.total = sum((len(_) for _ in dbs))
        self.board_size = dbs[0].board_size  # assume at least one db

    def is_single_db(self):
        """return true if it is flat"""
        return len(self.dbs) == 1
    ...
    def collate(self, indices):
        """Collate function for torch.utils.data.DataLoader."""
        indices_lst = []
        N, M = len(indices), len(self.dbs)
        for i in range(len(self.dbs)):
            items = np.array(
                indices[i * N // M : (i + 1) * N // M]
            )  # assuming dbs with equal length
            items %= len(self.dbs[i])
            indices_lst.append(items)

        tensors_lst = []
        for i in range(len(self.dbs)):
            data = self.dbs[i].collate(indices_lst[i])
            tensors_lst.append(data)

        return [
            self._concat(tensors_lst, _) for _ in range(len(tensors_lst[0]))
        ]

    def _concat(self, tensors_lst, idx):
        """concat with second dim"""
        tensors = [tensors_lst[_][idx] for _ in range(len(tensors_lst))]
        return torch.cat(tensors)

    def set_transform(self, seed: int | str | None):
        for db in self.dbs:
            db.set_transform(seed)
