...
class THTransformation:
    ...
    identity = TransformationFunction(
        f=lambda x: x, inv=lambda x: x, order=Order.CHW
    )
    rotate_90 = TransformationFunction(
        f=_th_rotate_90, inv=_th_rotate_270, order=Order.CHW
    )
    rotate_180 = TransformationFunction(
        f=_th_rotate_180, inv=_th_rotate_180, order=Order.CHW
    )
    rotate_270 = TransformationFunction(
        f=_th_rotate_270, inv=_th_rotate_90, order=Order.CHW
    )
    fliplr = TransformationFunction(
        f=_th_fliplr, inv=_th_fliplr, order=Order.CHW
    )
    flipud = TransformationFunction(
        f=_th_flipud, inv=_th_flipud, order=Order.CHW
    )
    diagonal_left_down = TransformationFunction(
        f=_th_diagonal_left_down, inv=_th_diagonal_left_down, order=Order.CHW
    )
    diagonal_right_down = TransformationFunction(
        f=_th_diagonal_right_down, inv=_th_diagonal_right_down, order=Order.CHW
    )
...
class TFTransformation:
    ...
    identity = TransformationFunction(
        f=lambda x: x, inv=lambda x: x, order=Order.HWC
    )
    rotate_90 = TransformationFunction(
        f=_tf_rotate_90, inv=_tf_rotate_270, order=Order.HWC
    )
    rotate_180 = TransformationFunction(
        f=_tf_rotate_180, inv=_tf_rotate_180, order=Order.HWC
    )
    rotate_270 = TransformationFunction(
        f=_tf_rotate_270, inv=_tf_rotate_90, order=Order.HWC
    )
    fliplr = TransformationFunction(
        f=_tf_fliplr, inv=_tf_fliplr, order=Order.HWC
    )
    flipud = TransformationFunction(
        f=_tf_flipud, inv=_tf_flipud, order=Order.HWC
    )
    diagonal_left_down = TransformationFunction(
        f=_tf_diagonal_left_down, inv=_tf_diagonal_left_down, order=Order.HWC
    )
    diagonal_right_down = TransformationFunction(
        f=_tf_diagonal_right_down, inv=_tf_diagonal_right_down, order=Order.HWC
    )
...
def index_to_onehot(action: int, board_size: int) -> np.ndarray:
    assert 0 <= action < board_size**2, (
        'Action index (%d) is out of bound of board size %d'
        % (action, board_size)
    )
...
def onehot_to_index(plane: np.ndarray, board_size: int) -> np.int64:
    return plane.reshape((board_size**2,)).argmax()


def transform_index(
    action: int, f: TransformationFunction, board_size: int
) -> np.int64:
    assert action is not None and action != -1 and action != board_size**2, (
        'Pass (%s) cannot be transformed' % action
    )
...
def transformation_functions(
    order: Order,
) -> Generator[Tuple[Dihedral8, TransformationFunction], None, None]:
    transformations = (
        TH_TRANSFORMATIONS if order == Order.CHW else TF_TRANSFORMATIONS
    )
...
def identical_transform_function(
    order: Order,
) -> Tuple[Dihedral8, TransformationFunction]:
    if order == Order.CHW:
...
def random_transform_function(
    order: Order, random_state=None
) -> Tuple[Dihedral8, TransformationFunction]:
...
def identical_transform_generator(
    order: Order,
) -> Generator[Tuple[Dihedral8, TransformationFunction], None, None]:
    while True:
...
def rotate_features_and_action(
    features: np.ndarray,
    action: int,
    board_size: int,
    f: TransformationFunction,
) -> Tuple[np.ndarray, int | np.int64]:
    features = f(features)
