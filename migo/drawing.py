...
def make_fig(board_size: int = 9, config: dict = {}, with_note: bool = False):
    """make plain figure
...
def draw_plane(ax, plane, color, config: dict = {}, scale: float = 1.0):
    ...
    ec, fc = (
        ((0.5, 0.5, 0.5), 'k') if int(color) == int(Color.BLACK) else ('k', 'w')
    )
...
def place_stones(
    ax, state: State | cygo.State, config: dict = {}, scale: float = 1.0
):
    """place stones on ax
...
def put_number(
    ...
    if with_shade:
        others['bbox'] = dict(
            facecolor=config['facecolor'], alpha=0.8, edgecolor='none'
        )
    ax.text(
        r,
        c - margin,
...
def plot_state(state, config: dict = {}):
    ...
    state_planes = features.board(state)
    black_index = (1 - int(state.current_player)) // 2
...
def plot_record(
    record, config: dict = {}, *, with_note: bool = True, overwrite: bool = True
):
    ...
    fig, ax, ax_note = make_fig(board_size, config=config, with_note=with_note)
    planes = np.zeros((2, board_size, board_size))
    if overwrite:
