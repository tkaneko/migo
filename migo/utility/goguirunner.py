...
class MigoGoGuiRunner(migo.gtp.GoGuiGTPRunner):
    def __init__(self, board_size):
        ...
        self.add_analyze_callback(
            migo.gtp.CommandType.BWBOARD, 'tromp_taylor', self.tromp_taylor_bw
        )
        self.add_analyze_callback(
            migo.gtp.CommandType.CBOARD, 'view_policy', self.view_policy
        )
        self.add_analyze_callback(
            migo.gtp.CommandType.CBOARD,
            'predict_territory',
            self.predict_territory,
        )
    ...
    def set_boardsize(self, *args):
        self.gtp_params.update('boardsize', int(args[0]))
        return migo.gtp.Status.success, self.gtp_params.boardsize

    def set_komi(self, *args):
        self.gtp_params.update('komi', float(args[0]))
        return migo.gtp.Status.success, self.gtp_params.komi

    def clear_board(self, *_args):
        self.game = cygo.State(
            board_size=self.gtp_params.boardsize, komi=self.gtp_params.komi
        )
    ...
    def gtp_play(self, *args):
        ...
        action = -1  # pass in cygo.State.make_move
        if args[1] != 'PASS':
    ...
    def tromp_taylor_bw(self, *_args):
    ...
    def make_normalized_color(self, policy):
        ...
        return [matplotlib.colors.to_hex(cmap(_)) for _ in policy]
    ...
    def predict_territory(self, *_args):
        if not self.ort_session:
            return migo.gtp.Status.failure, 'onnx not loaded yet'
